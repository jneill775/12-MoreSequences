...
def run_test_generate_points_on_circle():
    ...
    # Test 2:
    expected = [rg.Point(100, 250),  # All numbers are approximate.
                rg.Point(150, 300),
                rg.Point(50, 300),
                rg.Point(100, 350)]
...
def run_test_draw_points_on_circle():
    ...
    window = rg.RoseWindow(400, 400, title)
    circle = rg.Circle(rg.Point(200, 200), 150)
    draw_points_on_circle(window, circle, 200, 'yellow')
    window.close_on_mouse_click()
...
def draw_points_on_circle(window, circle, number_of_points, color):
    ...
    for k in range(len(points)):
        center = points[k]
        sperp = rg.Circle(center, 10)
        sperp.fill_color = color
        sperp.attach_to(window)
...
def run_test_pizza():
    ...
    # Test 4:
    title = 'PIZZA test 4:  150 slices, thin (thickness=1) black lines.'
    window = rg.RoseWindow(400, 400, title)
    circle = rg.Circle(rg.Point(200, 200), 150)
    circle.fill_color = 'yellow'
    circle.outline_thickness = 3
    pizza(window, circle, 150, 'black', 1)
    window.close_on_mouse_click()
...
def pizza(window, circle, number_of_slices, color, thickness):
    ...
    for k in range(number_of_slices + 1):
        waluigi = rg.Line(circle.center, pts[k-1])
        waluigi.color = color
        waluigi.thickness = thickness
        waluigi.attach_to(window)
        circle.attach_to(window)
        window.render()
